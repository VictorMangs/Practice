...
async fn main() -> Result<()> {
    ...
    // 🧹 Spawn cleanup handler for Ctrl+C or SIGTERM
    tokio::spawn(async {
        use tokio::signal;
        #[cfg(unix)]
        ...
        if Path::new("server.log").exists() {
            if let Err(e) = fs::remove_file("server.log") {
                eprintln!("❌ Failed to remove server.log: {:?}", e);
            } else {
                println!("🧹 server.log removed successfully.");
            }
        }
    ...
    loop {
        ...
        tokio::spawn(handle_client(stream, addr.to_string(), clients));
    }
}
